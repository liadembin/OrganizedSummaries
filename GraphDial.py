...
class GraphDialog(wx.Dialog):
    ...
    def _on_paint(self, event):
        ...
        for node_id, (x, y) in self.node_positions.items():
            ...
            name = current_node.name if current_node and hasattr(current_node, 'name') else str(node_id)
            if len(name) > 15:
                name = name[:12] + "..."
    ...
    def _on_view(self, evt):
       ...
       #  node = self.node_map.get(self.selected_id)
        ...
        msg = self.net.build_message("GETSUMMARY", [str(self.selected_id.id if hasattr(self.selected_id, 'id') else self.selected_id)])
