...
class TestHashFunctions:
    ...
    def test_hash_pass_consistent(self, crypt_manager, sample_data):
        ...
        hashed1 = crypt_manager.hash_pass(
            sample_data["password"], sample_data["salt"], sample_data["paper"]
        )
    ...
    def test_hash_pass_different_with_different_inputs(
        self, crypt_manager, sample_data
    ):
        ...
        # Different password
        hashed2 = crypt_manager.hash_pass(
            "different_password", sample_data["salt"], sample_data["paper"]
        )
        ...
        hashed3 = crypt_manager.hash_pass(
            sample_data["password"], different_salt, sample_data["paper"]
        )
        ...
        hashed4 = crypt_manager.hash_pass(
            sample_data["password"], sample_data["salt"], different_paper
        )
    ...
    def test_check_hash_valid(self, crypt_manager, sample_data):
        ...
        assert crypt_manager.check_hash(
            sample_data["password"], hashed, sample_data["salt"], sample_data["paper"]
    ...
    def test_check_hash_invalid(self, crypt_manager, sample_data):
        ...
        assert not crypt_manager.check_hash(
            "wrong_password", hashed, sample_data["salt"], sample_data["paper"]
...
class TestRSAEncryption:
    ...
    def test_rsa_encryption_decryption(self, crypt_manager, sample_data):
        ...
        assert (
            encrypted_data != sample_data["data"]
        ), "RSA encrypted data should differ from original"

        # Verify decryption returns original data
        decrypted_data = crypt_manager.decrypt_rsa(encrypted_data)
        assert (
            decrypted_data == sample_data["data"]
        ), "RSA decrypted data should match original"
...
class TestUtilityFunctions:
    ...
    def test_generate_random_bytes(self, crypt_manager):
        ...
        for length in lengths:
            ...
            assert (
                len(random_bytes) == length
            ), f"Random bytes should be {length} bytes long"
            ...
            assert (
                random_bytes != another_random
            ), "Random bytes should be different on each call"
